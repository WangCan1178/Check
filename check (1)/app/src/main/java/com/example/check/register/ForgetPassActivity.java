...
public class ForgetPassActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_password_first = findViewById(R.id.et_pass_first);
        et_password_first.setEditTextHint(getResources().getString(R.string.et_hint_pass));
        et_password_second = findViewById(R.id.et_pass_second);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.but_getCode) { // 点击了“获取验证码”按钮
            ...
            if (!isTelPhoneNumber(phone)) {
                ...
                Toast toast1 = toast.makeError(ForgetPassActivity.this,"手机号格式不正确，请重新输入");
            ...
            if (!et_code.getText().toString().equals(code)) {
                DynamicToast toast = new DynamicToast();
                Toast toast1 = toast.makeError(ForgetPassActivity.this,"验证码错误，请重新输入");
                toast1.setGravity(Gravity.TOP,0,50);
                toast1.show();
            }
            else if (!isPassword(password_first)) {
                DynamicToast toast = new DynamicToast();
                Toast toast1 = toast.makeError(ForgetPassActivity.this,"密码格式不正确，请重新输入");
                toast1.setGravity(Gravity.TOP,0,50);
                toast1.show();
                return;
            }
                ...
                DynamicToast toast = new DynamicToast();
                Toast toast1 = toast.makeError(ForgetPassActivity.this,"两次密码输入不相同，请重新输入");
                toast1.setGravity(Gravity.TOP,0,50);
                toast1.show();
                return;
            }
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ...
                        if(result.equals("success")){
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    DynamicToast toast = new DynamicToast();
                                    Toast toast1 = toast.makeSuccess(ForgetPassActivity.this,"修改成功，请登录");
                                    toast1.setGravity(Gravity.TOP,0,50);
                                    toast1.show();
