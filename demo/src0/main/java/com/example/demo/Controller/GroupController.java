...
public class GroupController {
    ...
    public String delAddGroup(@RequestBody Member member){
        int result = groupService.delAddGroup(member.getMemid(),member.getGroupid());
        if (result>0){
            return "success";
        }else {
            return "failed";
    ...
    public String addGroup(@RequestBody Member member){
        int result = groupService.addGroup(member.getMemid(),member.getGroupid(),member.getMemname(),1);
        if (result>0){
            return "success";
        }else {
            return "failed";
...
//    @ResponseBody
//    @GetMapping("/getTask")
//    public List<TaskUser> getTask(@RequestParam("id") int id,@RequestParam("userid") String userid){
//        List<TaskUser> result =  taskService.getTask(id,userid);
//        return result;
//    }
